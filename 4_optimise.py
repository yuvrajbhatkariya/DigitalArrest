...
# ⬇  Change this to your preferred ollama model (see header for recommendations)
LLM_MODEL          = "phi3:mini"

# History tuning
SUMMARY_THRESHOLD  = 6   # summarize once we have this many raw turns in the buffer
RECENT_TURNS_LIMIT = 4   # keep this many raw turns after summarization
MIN_AUDIO_SECONDS  = 0.5 # ignore audio chunks shorter than this
...
class ConversationMemory:
    ...
    _raw_buffer: list = field(default_factory=list)  # staging area before summarization
...
def detect_fraud(memory: ConversationMemory, current_turn: str) -> dict:
    ...
    try:
        resp = ollama.chat(
            ...
            options={
                "temperature": 0.0,   # deterministic for consistent verdicts
                "num_predict": 400,   # enough for JSON + reason fields
            },
            format="json",            # Ollama native JSON mode (model must support it)
        )
